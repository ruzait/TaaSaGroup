...
import { Globe, Truck, Package, TrendingUp, Check } from 'lucide-react'
...
function Navbar({ company }) {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${!isScrolled ? 'py-4' : 'py-2'}`}
      style={{ 
        backgroundColor: isScrolled ? `${COLORS.color}F2` : 'transparent',
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            <img src={Logo} alt="TaaSa" className="w-16 h-12 bg-white/90 rounded-lg p-1" />
            <span className="font-cursive font-bold text-xl text-white">{company.shortName}</span>
          </Link>
          <div className="hidden md:flex items-center gap-6">
            <Link to="#about" className="font-cursive text-white">About</Link>
            <Link to="#services" className="font-cursive text-white">Services</Link>
        ...
        >
          {isMobileMenuOpen ? (
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 18L18 6M6 6l12 12" /></svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12h18M3 6h18M3 18h18" /></svg>
          )}
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {COMPANIES.map((c) => (
                <li key={c.id}><Link to={`/company/${c.id}`} className="text-white/70 hover:text-white text-sm">{c.shortName}</Link></li>
              ))}
            </ul>
          </div>
          <div>
...
function WhatsAppButton({ company }) {
  return (
    <a href={`https://wa.me/${company?.whatsapp?.replace(/\s/g, '') || '94720516432'}`} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform" style={{ backgroundColor: '#25D366' }}>
