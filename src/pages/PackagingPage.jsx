...
import { Package, Check, Recycle, Truck, Shield, Leaf } from 'lucide-react'
    ...
    <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${!isScrolled ? 'py-4' : 'py-2'}`} style={{ backgroundColor: isScrolled ? `${COLORS.color}F2` : 'transparent', backdropFilter: isScrolled ? 'blur(12px)' : 'none' }}>
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link to="#products" className="font-cursive text-white">Products</Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 py-4 px-4 max-h-[calc(100vh-4rem)] overflow-y-auto" style={{ backgroundColor: COLORS.color }}>
          ...
          <Link to="#products" className="block font-cursive text-white py-3 border-b border-white/10" onClick={() => setIsMobileMenuOpen(false)}>Products</Link>
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto mt-20">
        ...
        <h1 className="font-cursive font-bold text-white text-5xl mb-4">{COMPANIES[2].shortName}</h1>
        <p className="font-handlee text-white/90 text-2xl mb-6">{COMPANIES[2].tagline}</p>
        <p className="font-nunito text-white/80 max-w-2xl mx-auto">{COMPANIES[2].description}</p>
      </div>
    ...
    <footer style={{ backgroundColor: COLORS.color }}>
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div><h4 className="font-handlee font-bold text-lg text-white mb-4">Contact</h4><p className="text-white/70 text-sm">{company?.address || 'Colombo, Sri Lanka'}</p><p className="text-white/70 text-sm">{company?.phone}</p><p className="text-white/70 text-sm">{company?.email}</p></div>
        </div>
