...
import { ArrowRight, Scale, Package, Home, Palmtree, Truck, Droplets, Sparkles, Award, Globe, Clock, Shield, Leaf, Users, TrendingUp } from 'lucide-react'
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full right-0 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                <div className="rounded-xl shadow-2xl overflow-hidden" style={{ backgroundColor: COLORS.color }}>
                  <Link to="/" className="block px-4 py-2 hover:bg-white/10 transition-colors whitespace-nowrap font-nunito text-sm border-b border-white/10" style={{ color: 'rgba(255,255,255,0.8)' }}>TaaSa Group Home</Link>
                  {COMPANIES.map((c) => (
                    <Link key={c.id} to={`/company/${c.id}`} className="block px-4 py-2 hover:bg-white/10 transition-colors whitespace-nowrap font-nunito text-sm" style={{ color: 'rgba(255,255,255,0.8)' }}>{c.shortName}</Link>
                  ))}
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <p className="text-white/70 text-sm">taasabusinessgroup@gmail.com</p>
        ...
        <div className="border-t border-white/10 mt-8 pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          {isScrolled && (
            <button onClick={scrollToTop} className="p-3 rounded-full transition-all hover:scale-110" style={{ backgroundColor: COLORS.accent }}>
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={COLORS.color} strokeWidth="2">
              ...
              return (
                ...
                >
                  <div className="flex flex-col lg:flex-row gap-0">
                    {/* Image Side */}
                    <div className="lg:w-1/2 h-[200px] sm:h-[250px] lg:h-[400px] relative overflow-hidden">
                      <motion.img 
